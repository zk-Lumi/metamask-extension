...
import { selectShowPrivacyPolicyToast, selectShowSurveyToast, } from './selectors';
describe('#getShowSurveyToast', () => {
    ...
    it('shows the survey link when not yet seen and within time bounds', () => {
        Date.now = () => new Date(`${SURVEY_DATE} 12:25:00 ${SURVEY_GMT}`).getTime();
    ...
    it('does not show the survey link before time bounds', () => {
        Date.now = () => new Date(`${SURVEY_DATE} 11:25:00 ${SURVEY_GMT}`).getTime();
        const result = selectShowSurveyToast({
            AppStateController: {
                surveyLinkLastClickedOrClosed: undefined
            }
    ...
    it('does not show the survey link after time bounds', () => {
        Date.now = () => new Date(`${SURVEY_DATE} 14:25:00 ${SURVEY_GMT}`).getTime();
        const result = selectShowSurveyToast({
            AppStateController: {
                surveyLinkLastClickedOrClosed: undefined
            }
