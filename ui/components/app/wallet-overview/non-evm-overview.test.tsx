...
const mockBuyableChainsWithoutBtc = defaultBuyableChains.filter((chain) => chain.chainId !== MultichainNetworks.BITCOIN);
...
describe('NonEvmOverview', () => {
    ...
    it('shows a spinner if balance is not available', async () => {
        const { container } = renderWithProvider(<NonEvmOverview>/>,, getStore({
            MultichainBalancesController: {
                // The balances won't be available
                balances: {}
            }
        }));
        const spinner = container.querySelector('.coin-overview__balance .coin-overview__primary-container .spinner');
    ...
    it('"Buy & Sell" button is enabled if BTC is buyable', () => {
        ...
        const { queryByTestId } = renderWithProvider(<NonEvmOverview>/>,, storeWithBtcBuyable);
        const buyButton = queryByTestId(BTC_OVERVIEW_BUY);
        expect(buyButton).toBeInTheDocument();
    ...
    it('sends an event when clicking the Buy button', () => {
        ...
        const { queryByTestId } = renderWithProvider(<MetaMetricsContext.Provider>value, { mockTrackEvent } >
            <NonEvmOverview>/>
            < /MetaMetricsContext.Provider>,, storeWithBtcBuyable);
        const buyButton = queryByTestId(BTC_OVERVIEW_BUY);
        expect(buyButton).toBeInTheDocument();
        expect(buyButton).not.toBeDisabled();
        fireEvent.click(buyButton as HTMLElement);
    ...
    it('sends an event when clicking the Send button', () => {
        ...
        const { queryByTestId } = renderWithProvider(<MetaMetricsContext.Provider>value, { mockTrackEvent } >
            <NonEvmOverview>/>
            < /MetaMetricsContext.Provider>,, getStore());
        const sendButton = queryByTestId(BTC_OVERVIEW_SEND);
        expect(sendButton).toBeInTheDocument();
        expect(sendButton).not.toBeDisabled();
