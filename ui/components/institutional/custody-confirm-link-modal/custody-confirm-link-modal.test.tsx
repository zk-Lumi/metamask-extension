...
describe('Custody Confirm Link', () => {
    ...
    it('tries to open new tab with deeplink URL', () => {
        ...
        const { getByTestId } = renderWithProvider(<CustodyConfirmLink>/>, store);, fireEvent.click(getByTestId('custody-confirm-link__btn')));
    ...
    it('shows custodian name when iconUrl is undefined', () => {
        ...
        const { getByText } = renderWithProvider(<CustodyConfirmLink>/>, store);, expect(getByText(mockedCustodianName)).toBeVisible());
    });
    it('shows text that comes from modal state if defined', () => {
        ...
        store = configureStore()(customMockStore);
        const { getByText } = renderWithProvider(<CustodyConfirmLink>/>, store);, expect(getByText(mockModalStateText)).toBeVisible());
    });
    it('shows action text that comes from modal state if defined', () => {
        ...
        store = configureStore()(customMockStore);
        const { getByText } = renderWithProvider(<CustodyConfirmLink>/>, store);, expect(getByText(mockModalStateActionText)).toBeVisible());
    });
});
