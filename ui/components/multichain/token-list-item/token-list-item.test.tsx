...
describe('TokenListItem', () => {
    ...
    it('should render correctly', () => {
        ...
        const { getByTestId, container } = renderWithProvider(<TokenListItem>{ ...props } /  > , store);
        expect(getByTestId('multichain-token-list-item')).toBeDefined();
        expect(container).toMatchSnapshot();
    });
    it('should render with custom className', () => {
        const store = configureMockStore()(state);
        const { getByTestId } = renderWithProvider(<TokenListItem>className, "multichain-token-list-item-test", { ...props } /  > , store);
        expect(getByTestId('multichain-token-list-item')).toHaveClass('multichain-token-list-item-test');
    });
    it('should render crypto balance with warning scam', () => {
        ...
        const { getByText } = renderWithProvider(<TokenListItem>{ ...propsToUse } /  > , store);
    ...
    it('should display warning scam modal', () => {
        ...
        const { getByTestId, getByText } = renderWithProvider(<TokenListItem>{ ...propsToUse } /  > , store);
        const warningScamModal = getByTestId('scam-warning');
        fireEvent.click(warningScamModal);
        expect(getByText('The native token symbol does not match the expected symbol of the native token for the network with the associated chain ID. You have entered SCAM_TOKEN while the expected token symbol is ETH. Please verify you are connected to the correct chain.')).toBeInTheDocument();
    });
    it('should display warning scam modal fallback when safechains fails to resolve correctly', () => {
        ...
        const warningScamModal = getByTestId('scam-warning');
        fireEvent.click(warningScamModal);
        expect(getByText('The native token symbol does not match the expected symbol of the native token for the network with the associated chain ID. You have entered SCAM_TOKEN while the expected token symbol is something else. Please verify you are connected to the correct chain.')).toBeInTheDocument();
    });
    ...
    it('handles click action and fires onClick', () => {
        const store = configureMockStore()(state);
        const { queryByTestId } = renderWithProvider(<TokenListItem>{ ...props }, tokenImage = "", title = "" /  > , store);
    ...
    it('handles clicking staking opens tab', async () => {
        const store = configureMockStore()(state);
        const { queryByTestId } = renderWithProvider(<TokenListItem>isStakeable, { ...props } /  > , store);
        const stakeButton = queryByTestId(`staking-entrypoint-${CHAIN_IDS.MAINNET}`);
