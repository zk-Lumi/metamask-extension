...
const renderUseMultichainHook = (selector: (state: MultichainState, account?: InternalAccount) => unknown, account?: InternalAccount, state?: MultichainState) => {
    return renderHookWithProvider(() => useMultichainSelector(selector, account ?? mockAccount), state ?? mockState);
};
describe('useMultichainSelector', () => {
    ...
    it('calls useSelector with the correct selector and account', () => {
        ...
        expect(mockSelector).toHaveBeenCalledWith(expect.anything(), // already checked above
        mockAccount);
    });
