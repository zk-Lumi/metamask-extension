...
import { useDisableProfileSyncing, useEnableProfileSyncing, useShouldDispatchProfileSyncing, } from './profileSyncing';
...
const arrangeMockState = (metamaskStateOverrides?: ArrangeMocksMetamaskStateOverrides) => {
    const state = {};
    return { state };
};
describe('useEnableProfileSyncing()', () => {
    it('should enable profile syncing', async () => {
        const mockEnableProfileSyncingAction = jest.spyOn(actions, 'enableProfileSyncing');
        const { state } = arrangeMockState();
        const { result } = renderHookWithProviderTyped(() => useEnableProfileSyncing(), state);
        await act(async () => {
...
describe('useDisableProfileSyncing()', () => {
    it('should disable profile syncing', async () => {
        const mockDisableProfileSyncingAction = jest.spyOn(actions, 'disableProfileSyncing');
        const { state } = arrangeMockState();
        const { result } = renderHookWithProviderTyped(() => useDisableProfileSyncing(), state, undefined, MetamaskNotificationsProvider);
        await act(async () => {
...
describe('useShouldDispatchProfileSyncing()', () => {
    ...
    it('should return true if all conditions are met', () => {
        ...
        const hook = renderHookWithProviderTyped(() => useShouldDispatchProfileSyncing(), state);
    ...
    testCases.failureStateCases.forEach(({ state, failingField }) => {
        it(`should return false if not all conditions are met [${failingField} = false]`, () => {
            ...
            const hook = renderHookWithProviderTyped(() => useShouldDispatchProfileSyncing(), newState);
