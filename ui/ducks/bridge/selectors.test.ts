...
import { BUILT_IN_NETWORKS, CHAIN_IDS, FEATURED_RPCS, } from '../../../shared/constants/network';
import { ALLOWED_BRIDGE_CHAIN_IDS, BRIDGE_QUOTE_MAX_ETA_SECONDS, } from '../../../shared/constants/bridge';
...
import { getAllBridgeableNetworks, getBridgeQuotes, getFromAmount, getFromChain, getFromChains, getFromToken, getFromTokens, getFromTopAssets, getIsBridgeTx, getToChain, getToChains, getToToken, getToTokens, getToTopAssets, } from './selectors';
describe('Bridge selectors', () => {
    ...
    describe('getAllBridgeableNetworks', () => {
        it('returns list of ALLOWED_BRIDGE_CHAIN_IDS networks', () => {
            ...
            expect(result[0]).toStrictEqual(expect.objectContaining({ chainId: FEATURED_RPCS[0].chainId }));
            expect(result[1]).toStrictEqual(expect.objectContaining({ chainId: FEATURED_RPCS[1].chainId }));
            FEATURED_RPCS.forEach((rpcDefinition, idx) => {
                expect(result[idx]).toStrictEqual(expect.objectContaining({
                    ...rpcDefinition,
                    blockExplorerUrls: [
        ...
        it('returns network if included in ALLOWED_BRIDGE_CHAIN_IDS', () => {
            ...
            expect(result[0]).toStrictEqual(expect.objectContaining({ chainId: CHAIN_IDS.MAINNET }));
            expect(result[1]).toStrictEqual(expect.objectContaining({ chainId: CHAIN_IDS.LINEA_MAINNET }));
            expect(result.find(({ chainId }) => chainId === CHAIN_IDS.MOONBEAM)).toStrictEqual(undefined);
        });
    });
    ...
    describe('getToChains', () => {
        it('excludes selected providerConfig and disabled chains from options', () => {
            ...
            expect(result[0]).toStrictEqual(expect.objectContaining({ chainId: CHAIN_IDS.ARBITRUM }));
            expect(result[1]).toStrictEqual(expect.objectContaining({ chainId: CHAIN_IDS.OPTIMISM }));
            expect(result[2]).toStrictEqual(expect.objectContaining({ chainId: CHAIN_IDS.POLYGON }));
        });
        it('returns empty list when bridgeFeatureFlags are not set', () => {
            const state = createBridgeMockStore();
            const result = getToChains(state as never);
    ...
    describe('getIsBridgeTx', () => {
        ...
        it('returns true if bridge is enabled and fromChain and toChainId have different chainIds', () => {
            const state = createBridgeMockStore({
                ...
                bridgeSliceOverrides: { toChainId: '0x38' },
                metamaskStateOverrides: {
                    ...mockNetworkState(...Object.values(BUILT_IN_NETWORKS), ...FEATURED_RPCS.filter((network) => network.chainId !== CHAIN_IDS.LINEA_MAINNET)),
    ...
    describe('getBridgeQuotes', () => {
        ...
        it('should sort quotes by adjustedReturn', () => {
            ...
            const { activeQuote, recommendedQuote, sortedQuotes } = getBridgeQuotes(state as never);
            ...
            expect(quoteMetadataKeys.every((k) => Object.keys(activeQuote ?? {}).includes(k))).toBe(true);
            expect(activeQuote?.quote.requestId).toStrictEqual('381c23bc-e3e4-48fe-bc53-257471e388ad');
            expect(recommendedQuote?.quote.requestId).toStrictEqual('381c23bc-e3e4-48fe-bc53-257471e388ad');
            expect(sortedQuotes).toHaveLength(2);
            sortedQuotes.forEach((quote, idx) => {
                expect(quoteMetadataKeys.every((k) => Object.keys(quote ?? {}).includes(k))).toBe(true);
                expect(quote?.quote.requestId).toStrictEqual(mockBridgeQuotesNativeErc20[idx]?.quote.requestId);
            });
        });
        it('should sort quotes by ETA', () => {
            ...
            expect(sortedQuotes[1]?.quote.requestId).toStrictEqual(mockBridgeQuotesNativeErc20[1]?.quote.requestId);
            expect(sortedQuotes[2]?.quote.requestId).toStrictEqual(mockBridgeQuotesNativeErc20[0]?.quote.requestId);
        });
        it('should recommend 2nd cheapest quote if ETA exceeds 1 hour', () => {
            const state = createBridgeMockStore({
                ...
                bridgeStateOverrides: {
                    quotes: [
                        ...
                        {
                            ...mockBridgeQuotesNativeErc20[0],
                            estimatedProcessingTimeInSeconds: BRIDGE_QUOTE_MAX_ETA_SECONDS + 1,
                            quote: {
                                ...mockBridgeQuotesNativeErc20[0].quote,
            ...
            const { activeQuote, recommendedQuote, sortedQuotes } = getBridgeQuotes(state as never);
            expect(activeQuote?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(recommendedQuote?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(sortedQuotes).toHaveLength(2);
            expect(sortedQuotes[0]?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(sortedQuotes[1]?.quote.requestId).toStrictEqual('cheapestQuoteWithLongETA');
        });
        it('should recommend 2nd fastest quote if adjustedReturn is less than 80% of cheapest quote', () => {
            ...
            const { activeQuote, recommendedQuote, sortedQuotes } = getBridgeQuotes(state as never);
            const { sentAmount, totalNetworkFee, toTokenAmount, adjustedReturn, cost, } = activeQuote ?? {};
            expect(activeQuote?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(recommendedQuote?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(sentAmount?.fiat?.toString()).toStrictEqual('25.2425');
            expect(totalNetworkFee?.fiat?.toString()).toStrictEqual('2.52459306428938562');
            expect(toTokenAmount?.fiat?.toString()).toStrictEqual('24.226654664163');
            expect(adjustedReturn?.fiat?.toString()).toStrictEqual('21.70206159987361438');
            expect(cost?.fiat?.toString()).toStrictEqual('3.54043840012638562');
            expect(sortedQuotes).toHaveLength(3);
            expect(sortedQuotes[0]?.quote.requestId).toStrictEqual('fastestQuote');
            expect(sortedQuotes[1]?.quote.requestId).toStrictEqual('4277a368-40d7-4e82-aa67-74f29dc5f98a');
            expect(sortedQuotes[2]?.quote.requestId).toStrictEqual('381c23bc-e3e4-48fe-bc53-257471e388ad');
        });
    });
});
