...
function runHook({ currentConfirmation, transactions = [], }: {
    ...
    const response = renderHookWithConfirmContextProvider(useSigningOrSubmittingAlerts, state);
...
describe('useSigningOrSubmittingAlerts', () => {
    ...
    it('returns no alerts if no transactions', () => {
        expect(runHook({ currentConfirmation: CONFIRMATION_MOCK, transactions: [] })).toEqual([]);
    });
