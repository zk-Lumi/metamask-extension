...
import { getMockConfirmState, getMockPersonalSignConfirmState, getMockPersonalSignConfirmStateForRequest, getMockTypedSignConfirmStateForRequest, } from '../../../../../../../test/data/confirmations/helper';
...
jest.mock('../../../../../../../node_modules/@metamask/snaps-utils', () => {
    const originalUtils = jest.requireActual('../../../../../../../node_modules/@metamask/snaps-utils');
    return {
        ...originalUtils,
...
describe('PersonalSignInfo', () => {
    it('renders correctly for personal sign request', () => {
        ...
        const { container } = renderWithConfirmContextProvider(<PersonalSignInfo>/>,, mockStore);
    ...
    it('display signing in from for SIWE request', () => {
        ...
        const state = getMockPersonalSignConfirmStateForRequest(signatureRequestSIWE);
        const mockStore = configureMockStore([])(state);
        const { getByText } = renderWithConfirmContextProvider(<PersonalSignInfo>/>,, mockStore);
    ...
    it('does not display tooltip text when isSIWE is true', async () => {
        const state = getMockPersonalSignConfirmStateForRequest(signatureRequestSIWE); // isSIWE is true
        (utils.isSIWESignatureRequest as jest.Mock).mockReturnValue(true);
        const mockStore = configureMockStore([])(state);
        const { queryByText, getByText } = renderWithConfirmContextProvider(<PersonalSignInfo>/>,, mockStore);
        const requestFromLabel = getByText('Request from');
        await requestFromLabel.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
        expect(queryByText('This is the site asking for your signature.')).toBeNull();
        expect(queryByText('This is the Snap asking for your signature.')).toBeNull();
    });
    ...
    it('displays "requestFromInfo" tooltip when isSIWE is false and origin is not a snap', async () => {
        ...
        (snapUtils.isSnapId as jest.Mock).mockReturnValue(true);
        const mockStore = configureMockStore([])(state);
        const { getByText, queryByText } = renderWithConfirmContextProvider(<PersonalSignInfo>/>,, mockStore);
        const requestFromLabel = getByText('Request from');
        await requestFromLabel.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
        expect(queryByText('This is the site asking for your signature.')).toBeDefined();
    });
});
