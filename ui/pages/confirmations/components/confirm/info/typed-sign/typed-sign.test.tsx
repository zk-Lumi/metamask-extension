...
import { getMockConfirmStateForTransaction, getMockTypedSignConfirmState, getMockTypedSignConfirmStateForRequest, } from '../../../../../../../test/data/confirmations/helper';
import { permitSignatureMsg, permitSignatureMsgWithNoDeadline, unapprovedTypedSignMsgV3, unapprovedTypedSignMsgV4, } from '../../../../../../../test/data/confirmations/typed_sign';
...
describe('TypedSignInfo', () => {
    it('renders origin for typed sign data request', () => {
        ...
        const { container } = renderWithConfirmContextProvider(<TypedSignInfo>/>,, mockStore);
    ...
    it('should render message for typed sign v3 request', () => {
        const state = getMockTypedSignConfirmStateForRequest(unapprovedTypedSignMsgV3);
        const mockStore = configureMockStore([])(state);
        const { container } = renderWithConfirmContextProvider(<TypedSignInfo>/>,, mockStore);
        expect(container).toMatchSnapshot();
    });
    ...
    it('display simulation details for permit signature if flag useTransactionSimulations is set', () => {
        ...
        const mockStore = configureMockStore([])(state);
        const { getByText } = renderWithConfirmContextProvider(<TypedSignInfo>/>,, mockStore);
    ...
    it('displays "requestFromInfoSnap" tooltip when origin is a snap', async () => {
        ...
        const { queryByText } = renderWithConfirmContextProvider(<TypedSignInfo>/>,, mockStore);
